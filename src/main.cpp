...
uint256 hashGenesisBlock("0xe9b902838cd699a0febdba44a8172e52c72d7d432fbc887ec721272e552f7a7b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Mycoin: starting difficulty is 1 / 2^12
...
{
    // Mycoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Mycoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Mycoin: 3.5 days
static const int64 nTargetSpacing = 1 * 60; // Mycoin: 2.5 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Mycoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x7f6b297fd111002f91b886eb874822c88dc0f601c77936eca14c88ece82e9f5e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 56";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04fa6bf80cdc5a9ab3fe38007883ab6a99a0ef43daf7576d7c8220c5ec780fe2d6a43e05f6d04c9c1013112246f846f1a986eb0f2d6b72852f27c61374c88c07bc") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1525487415;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085371869;

        if (fTestNet)
        {
            block.nTime    = 1525487399;
            block.nNonce   = 385459868;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xe15656835d7eaac7d2aa8f24afa4cf9ea0be81fa4418ac4c66b58b02f19867a6"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xee, 0xcc, 0xbb, 0xaa }; // Mycoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MycoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MycoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MycoinMiner(CWallet *pwallet)
{
    printf("MycoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("mycoin-miner");
    ...
    try { loop {
        ...
        printf("Running MycoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MycoinMiner, pwallet));
}
